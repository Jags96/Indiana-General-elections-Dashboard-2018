...
summary_ui <- function(id, parties_map, counties_vector) {
  ns <- NS(id)
  ...
  layout_columns(
    ...
    card(
      card(
        card_header("Choose Parties:"), checkboxGroupInput(
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      output$PartyWise_plot <- renderPlot({
        ggplot(filtered_county_votes_df(), mapping = aes(y = total_votes, x = party_simplified)) +
          geom_col() +
          labs(
      ...
      output$CountyWise_plot <- renderPlot({
        filtered_party_county_df() |> ggplot(mapping = aes(y = total_votes, x = county_name)) +
          geom_col() +
          labs(
